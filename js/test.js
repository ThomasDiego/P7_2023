...
function searchRecipesFunctional(recipes, searchParams) {
  ...
  const filteredRecipes = recipes.filter((recipe) => {
    if (search) {
      ...
      const matchedSearchTerms = searchTerms.filter(
        (term) =>
          recipe.name.toLowerCase().includes(term) ||
          recipe.description.toLowerCase().includes(term)
      );

      if (matchedSearchTerms.length !== searchTerms.length) {
        return false;
      }
    }
    ...
    if (ustensils) {
      const matchedUstensils = ustensils.filter((ustensil) =>
        recipe.ustensils.includes(ustensil)
      );

      if (matchedUstensils.length !== ustensils.length) {
        return false;
      }
    }
...
const searchParams = {
  search: "pate",
  ingredients: ["citron"],
  ustensils: ["moule à tarte"],
  appliance: "four",
};

console.log(searchParams);
console.time("searchRecipesLoop");
const test1 = searchRecipesLoop(recipes, searchParams);
if (test1.length > 0) {
  console.log(test1[0].name);
}
console.log(test1.length + " résultat(s) ");
console.timeEnd("searchRecipesLoop");

console.time("searchRecipesFunctional");
const test2 = searchRecipesFunctional(recipes, searchParams);
if (test2.length > 0) {
  console.log(test2[0].name);
}
console.log(test2.length + " résultat(s)");
console.timeEnd("searchRecipesFunctional");
